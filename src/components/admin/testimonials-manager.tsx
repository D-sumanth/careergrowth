...
const defaultForm = { name: "", role: "", content: "", rating: "5" };
...
export function TestimonialsManager({ items }: { items: TestimonialRecord[] }) {
  ...
  const [form, setForm] = useState(
    items[0]
      ? { name: items[0].name, role: items[0].role, content: items[0].content, rating: String(items[0].rating ?? 5) }
      : defaultForm,
  );
  ...
  function select(item: TestimonialRecord | null) {
    ...
    setForm(item ? { name: item.name, role: item.role, content: item.content, rating: String(item.rating ?? 5) } : defaultForm);
    setStatus("");
    setError("");
  }
