...
export default async function Home() {
  ...
  return (
    <>
      <SiteHeader />
      <main>
        <section className="mx-auto max-w-7xl px-4 py-9 sm:px-6 sm:py-14 lg:px-8 lg:py-16">
          <div className="grid gap-7 lg:grid-cols-[1.18fr_0.82fr] lg:items-center">
            <div className="space-y-5">
              <Badge className="bg-amber-100 text-amber-900">{siteConfig.heroBadge}</Badge>
              <div className="space-y-3">
                <h1 className="max-w-4xl font-serif text-[2.35rem] font-semibold leading-tight tracking-tight text-slate-950 sm:text-5xl lg:text-[3.65rem]">
          ...
          <div className="mt-7 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            {featuredServices.map((service) => (
              <Card key={service.slug} className="overflow-hidden">
                ...
                <div className={`p-5 ${service.imageUrl ? "bg-white" : `bg-gradient-to-br ${service.accent}`}`}>
                  <h3 className="font-serif text-2xl font-semibold leading-tight text-slate-950">{service.title}</h3>
          ...
          <div className="mt-7 grid gap-4 md:grid-cols-3">
            {[
              ...
              <Card key={title} className="p-5">
                <div className="mb-4 inline-flex h-9 w-9 items-center justify-center rounded-lg bg-amber-100 text-sm font-semibold text-amber-900">{index + 1}</div>
              ...
              <Card className="bg-slate-950 p-6 text-white sm:p-8">
                ...
                <div className="mt-5 space-y-3">
                  {workshops.slice(0, 2).map((workshop) => (
                    <div key={workshop.slug} className="rounded-lg border border-white/10 bg-white/5 p-4">
                      {workshop.imageUrl ? (
                        // eslint-disable-next-line @next/next/no-img-element
                        <img src={workshop.imageUrl} alt={workshop.title} className="mb-4 aspect-video w-full rounded-lg object-cover" />
                      ) : null}
                      <p className="text-xs font-semibold uppercase tracking-[0.14em] text-amber-200">{workshop.status.replaceAll("_", " ")}</p>
          ...
          <div className="mt-7 grid gap-4 md:grid-cols-3">
            {blogPosts.map((post) => (
              <Card key={post.slug} className="p-5">
                <p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">{post.topic}</p>
                <h3 className="mt-3 font-serif text-2xl font-semibold leading-tight text-slate-950">{post.title}</h3>
